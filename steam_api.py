...
class SteamAPI:
    ...
    def get_owned_games(self, steamid: str) -> list[dict]:
        ...
        params = { "key": self.key, "steamid": steamid, "include_appinfo": True, "include_played_free_games": True}
    ...
    def get_recently_played_games(self, steamid: str, count: int = 50) -> list[dict]:
        ...
        params = {"key": self.key, "steamid": steamid, "count": count}
        r = requests.get(url, params=params, timeout=10)
        r.raise_for_status()
        return r.json().get("response", {}).get("games", [])
    ...
    def get_friends(self, steamid: str) -> list[str]:
        ...
        params = {"key": self.key, "steamid": steamid, "relationship": "friend"}
        r = requests.get(url, params=params, timeout=10)
        r.raise_for_status()
    ...
    def get_app_details(self, appid: int) -> dict:
        ...
        return {"appid": appid, "name": data.get("name", ""), "description": data.get("short_description", ""), "genres": ",".join(g.get("description", "") for g in data.get("genres", []))}
